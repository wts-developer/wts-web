  ...
  function runCalculator(root, app) {
    ...
    const SCHOLARSHIPS = {
      MATS: [
        ...
        { id: "missionary", calc: { type: "percentTuition", pct: 0.20 }, name: "Missionary Scholarship", detail: "20% tuition scholarship for active missionaries, or those entering the field within 1 to 2 years, verified by a sending missions agency." },
        { id: "educator", calc: { type: "percentTuition", pct: 0.20 }, name: "Christian Educator Scholarship", detail: "20% tuition scholarship for those employed full-time in an educational setting: a school, homeschool program, educational resource organization, or church." },
        { id: "churchOfficer", calc: { type: "percentTuition", pct: 0.20 }, name: "Church Officer Scholarship", detail: "20% tuition scholarship for active church officers (teaching or ruling elders, pastors, deacons) in any denomination, confirmed by pastoral reference." }
      ],
      MAC: [
    ...
    function calculate() {
      ...
      // Tiles use the same fee-inclusive figures as the pie: tuition
      // before support counts the SBC recognition fee for every course
      // selected for transfer (the fee is owed regardless of the credit
      // reduction), and WTS support counts the fee scholarship, so the
      // four tiles keep their arithmetic identity.
      els.miniMatch.textContent = money(pieWtsScholarshipSupport);
      els.miniRemaining.textContent = money(fundsApplied);
      els.miniGross.textContent = money(pieTotal);
    ...
    root.querySelectorAll(".alt-modality[data-jump]").forEach(chip => {
    ...
    const contactButton = root.querySelectorAll(".contact-cta .contact-button")[0];
    if (contactButton) {
