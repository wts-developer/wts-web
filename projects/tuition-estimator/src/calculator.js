    ...
    function calculate() {
      ...
      if (program.funded) {
        ...
        // amount and a $0 cost; the inputs panel is hidden in this mode.
        const rows = buildTerms(program.credits, 3, els.startTerm.value, program.termSystem);
        ...
        els.termTable.innerHTML = rows.map(row => `
          <tr>
            ...
            <td class="money">$0</td>
            <td class="money">${money(row.tuition)}</td>
            <td class="money"><strong>$0</strong></td>
          </tr>
        `).join("");
